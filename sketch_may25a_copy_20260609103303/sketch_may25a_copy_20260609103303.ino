...
const char* DEVICE_MAC = "f4:41:0d:30:6e:f7";

// ── Intervalos ───────────────────────────────────────────────────────────────
const unsigned long MEASURE_INTERVAL_MS = 5UL * 60UL * 1000UL;  // 5 min
...
bool connectToBracelet() {
  ...
  mac.toLowerCase();
  if (!bleClient->connect(BLEAddress(mac.c_str()), BLE_ADDR_TYPE_RANDOM)) return false;
...
void setup() {
  ...
  if (!connectToBracelet()) {
    Serial.println("[BLE] Falha na conexão. Reiniciando...");
    delay(3000);
    ESP.restart();
  }
        ...
        if (phaseRetries >= MAX_PHASE_RETRIES) {
          if (canAdvanceFromPhase(readPhase)) {
            if (readPhase < 2 && !captured[readPhase]) {
              Serial.printf("[APP] Sinal obrigatório %s não capturado. Reiniciando...\n",
